...
class Utterance:
    def __init__(self, utterance_index, utterance_string, speaker_id, terminator_type, tokenizer=None, nlp=None):
        ...
        self.verb_type = None
        self.control_verb = None

        self.create_utterance(nlp)
        self.verb_type = self.classify_main_verb(self.lemma)
        self.control_verb = self.classify_main_verb(self.lemma)
        ...
        if self.tokenizer is not None:
            if self.tokenizer == 'spacy':
                if token.get('dep') == 'ROOT':
                    ...
                    if self.main_verb == "'m":
                        self.main_verb = "am"
                    if self.main_verb == "'re":
                        self.main_verb = "are"
                    if self.main_verb == "'ve":
                        self.main_verb = "have"
                    if self.main_verb == "'ll":
                        self.main_verb = "will"
                    if token.get('lemma') is not None:
                        self.lemma = token.get('lemma')
    ...
    def classify_main_verb(self, lemma):
        vendler_classification = {
            ...
            "admire": "activity/state",
            # He admired her artwork for twenty minutes (act); I admire your work ethic (state)
            ...
            "assume": "activity/achievement",
            # I assumed the worst for a short while (act); I assumed it was over (ach)
            ...
            "bait": "activity/accomplishment",
            # he baited her for years (act); he baited the fishing pole in ten seconds (acc)
            ...
            "become": "achievement/accomplishment",
            # He became angry in twenty seconds (acc); He became a god instantly (ach);
            ...
            "bitch": "activity",
            # She bitched about it for a while (act); he managed to bend the metal in twelve minutes (acc)
            ...
            "blind": "activity/achievement",
            # He blinded her with a blindfold for a few minutes (act); He blinded her (ach)
            ...
            "board": "achievement/accomplishment",
            # He boarded the plane (ach); He boarded up the door in ten minutes (act)
            ...
            "capture": "achievement/accomplishment",
            # He captured the bug in two minutes (acc); He captured a photo (ach)
            "cash": "accomplishment",  # they cashed her check in five minutes (acc)
            "cashier": "activity",  # He cashiered for his whole shift (act);
            "cast": "achievement/accomplishment",
            # He cast a spell on her (ach); He cast his vote in twenty minutes (acc)
            ...
            "chase": "activity/accomplishment",
            # he chased her for a while (act); he chased her out in five minutes (acc)
            ...
            "claim": "activity/achievement",
            # he claimed that he was innocent for hours (act); he claimed his reward (ach)
            ...
            "care": "activity/state",
            # He cared for her for years (act); I care about you; *I am caring about you (state)
            ...
            "change": "achievement/accomplishment",
            # He changed the song (ach); he changed his clothes in five seconds (acc)
            ...
            "contain": "achievement/accomplishment",
            # the boxes contain drugs (ach); he contained the fire in five days (acc)
            ...
            "drill": "accomplishment",  # he drilled all the screws in x time (acc)
            ...
            "erupt": "activity/achievement",
            # the volcano erupted for a few minutes (act); it erupted into flames (ach)
            ...
            "hate": "activity/state",
            # I hated this game for years (act); I hate this game; *I am hating this game (state)
            "have": "activity/state",
            # I had this game for years (act); I have this game; *I am having this game (state)
            ...
            "jam": "activity/achievement",
            # We jammed together for five hours (act); He jammed his foot into the wall (ach)
            ...
            "lug": "activity/accomplishment",
            # he lugged the box into the house for/in ten minutes (activity/accomplishment)
            ...
            "move": "activity/accomplishment",
            # he moved around all day (act)/ me moved into a new house in ten hours (acc)
            ...
            "open": "achievement/accomplishment",
            # He opened his eyes (ach); He opened the present in a few minutes (acc)
            ...
            "slash": "achievement/accomplishment",
            # he slashed her tire (ach); he slashed all her tires in ten minutes (acc)
            "sled": "activity",  # act
            ...
            "steam": "activity",
            # He steamed the clothes for a bit (act); He steamed all the clothes in x minutes (acc)
            ...
            "wander": "activity/accomplishment",
            # he wandered for a while (act); he wandered home in five minutes (acc)
        ...
        self.verb_type = vendler_classification.get(lemma, "unknown")
